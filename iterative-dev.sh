...
function killServerProcesses() {
    pid="$(pgrep [PROJ_NAME_PLACEHOLDER])"
    if [ "$pid" != "" ]
    then
        echo "killing [PROJ_NAME_PLACEHOLDER]"
        kill $pid
    fi
